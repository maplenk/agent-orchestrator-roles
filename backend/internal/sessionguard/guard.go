...
// DeliverHost is for AO-owned injection (after-start switch handoff prompt).
// It still refuses terminated/exited sessions, but allows delivery while
...
func (g *Guard) DeliverWithPostWrite(ctx context.Context, id domain.SessionID, msg string, after func(context.Context) error) (Outcome, error) {
	return g.sendThen(ctx, id, msg, originUser, func(rec domain.SessionRecord) (Outcome, bool) {
		return SuppressedAwaitingUser, rec.Activity.State == domain.ActivityBlocked
	}, after)
}
...
func (g *Guard) DeliverUnderMutation(ctx context.Context, id domain.SessionID, msg string) (Outcome, error) {
	return g.sendAdmitted(ctx, id, msg, originHost, func(rec domain.SessionRecord) (Outcome, bool) {
		return SuppressedAwaitingUser, rec.Activity.State == domain.ActivityBlocked
	})
}
...
func (g *Guard) coordinationUnderMutation(
	...
	return g.sendAdmittedChecked(ctx, id, msg, originAuto, func(rec domain.SessionRecord) (Outcome, bool) {
...
func (g *Guard) send(ctx context.Context, id domain.SessionID, msg string, origin writeOrigin, refuse func(domain.SessionRecord) (Outcome, bool)) (Outcome, error) {
	return g.sendThen(ctx, id, msg, origin, refuse, nil)
}

func (g *Guard) sendThen(ctx context.Context, id domain.SessionID, msg string, origin writeOrigin, refuse func(domain.SessionRecord) (Outcome, bool), after func(context.Context) error) (Outcome, error) {
	g.leaseMu.RLock()
	lease := g.lease
	g.leaseMu.RUnlock()
	if lease != nil {
	...
	outcome, err := g.sendAdmitted(ctx, id, msg, origin, refuse)
...
func (g *Guard) sendAdmitted(ctx context.Context, id domain.SessionID, msg string, origin writeOrigin, refuse func(domain.SessionRecord) (Outcome, bool)) (Outcome, error) {
	return g.sendAdmittedChecked(ctx, id, msg, origin, refuse, nil)
}

func (g *Guard) sendAdmittedChecked(ctx context.Context, id domain.SessionID, msg string, origin writeOrigin, refuse func(domain.SessionRecord) (Outcome, bool), preWrite func(context.Context, domain.SessionRecord) error) (Outcome, error) {
