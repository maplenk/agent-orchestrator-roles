...
func TestGetLaunchCommandPrefersSystemPromptFileWhenBothAreSupplied(t *testing.T) {
	plugin := &Plugin{resolvedBinary: "codex"}
	promptFile := filepath.Join("tmp", "prompt with spaces.md")
	cmd, err := plugin.GetLaunchCommand(context.Background(), ports.LaunchConfig{
		SystemPromptFile: promptFile,
	...
	if !containsSubsequence(cmd, []string{"-c", "model_instructions_file=" + promptFile}) || contains(cmd, "developer_instructions="+codexTOMLConfigString("large inline prompt must stay out of argv")) {
		t.Fatalf("command %#v did not prefer the system prompt file", cmd)
	}
}

func TestGetLaunchCommandUsesInlineSystemPromptAsFallback(t *testing.T) {
	plugin := &Plugin{resolvedBinary: "codex"}
	cmd, err := plugin.GetLaunchCommand(context.Background(), ports.LaunchConfig{SystemPrompt: "inline fallback"})
	if err != nil {
		t.Fatal(err)
	}
	if !containsSubsequence(cmd, []string{"-c", "developer_instructions=" + codexTOMLConfigString("inline fallback")}) {
...
func TestGetRestoreCommandUsesInlineSystemPromptAsFallback(t *testing.T) {
	...
	if !containsSubsequence(cmd, []string{"-c", "developer_instructions=" + codexTOMLConfigString("restore inline fallback")}) {
