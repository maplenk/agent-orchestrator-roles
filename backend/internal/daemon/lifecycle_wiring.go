...
func startSession(cfg config.Config, runtime runtimeselect.Runtime, store *sqlite.Store, lcm *lifecycle.Manager, messenger ports.AgentMessenger, telemetry ports.EventSink, agents ports.AgentResolver, previewLifecycle sessionmanager.PreviewLifecycle, browserLifecycle sessionmanager.BrowserLifecycle, browserCapabilities sessionmanager.BrowserCapabilityIssuer, spawnCapabilities sessionmanager.SpawnCapabilityIssuer, log *slog.Logger) (*sessionsvc.Service, reviewsvc.Manager, sessionLifecycle, error) {
	...
	mgr := sessionmanager.New(sessionmanager.Deps{
		...
		SpawnCapabilities:   spawnCapabilities,
		DataDir:             cfg.DataDir,
		Logger:              log,
	})
