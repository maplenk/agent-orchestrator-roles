...
- After spawning any role worker, do not yield and assume AO will wake you: check `+"`ao session get <worker-session-id> --json`"+` at bounded intervals of no more than 60 seconds until roleResult.current=true (completed, blocked, or failed). AO messages may carry detail, but are not the semantic completion signal. Idle alone is not completion. A reviewer or verifier's terminal-only report may not emit an AO completion message, so retrieve `+"`ao session output <worker-session-id>`"+` before synthesizing that result. If a worker exits without a current role result, retrieve output only for diagnosis and report the missing result rather than inferring success.
...
5. Retain coordination ownership for every worker and check `+"`ao session get <worker-session-id> --json`"+` at bounded intervals of no more than 60 seconds instead of yielding and assuming an automatic wake-up. A current completed/blocked/failed roleResult is the semantic signal; idle and ordinary AO messages are not. Monitor PR state, CI, and reviews alongside that result. For a reviewer or verifier whose report can be terminal-only, retrieve the detailed report with `+"`ao session output <worker-session-id>`"+` before synthesizing.
