...
func (m *Manager) prepareTargetActivation(ctx context.Context, store ports.AgentSwitchStore, rec domain.SessionRecord, project domain.ProjectRecord, agent ports.Agent, caps ports.ContinuationCapabilities, sw domain.AgentSwitch, requiredGeneration domain.AgentGenerationID) (preparedTargetActivation, error) {
	if strings.TrimSpace(string(requiredGeneration)) == "" {
		return preparedTargetActivation{}, errors.New("required target generation is empty")
	}
	...
	argv, rawLaunchID, err := m.superviseAgentProcessForSwitch(agent, rec.ID, env, argv, string(requiredGeneration))
	if err != nil {
