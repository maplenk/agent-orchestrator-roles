...
type chatSpawn struct {
	...
	prompt           string
	systemPrompt     string
	// agentConfig is the ROLE-MERGED config, resolved once by the caller.
	// Reading the project config here instead would drop the role's model and
	// permissions on every chat spawn.
	agentConfig ports.AgentConfig
	// spawnToken is the session-scoped spawn capability. A chat agent gets one
	// for the same reason a terminal agent does: without it the session cannot
	// authenticate a spawn, and the persisted hash would match nothing.
	spawnToken string
}
...
func (m *Manager) launchChatController(ctx context.Context, in chatSpawn) (domain.SessionRecord, error) {
	...
	env := m.runtimeEnv(id, in.cfg.ProjectID, in.cfg.IssueID, in.project.Config.Env, in.spawnToken)
...
func (m *Manager) resumeChatController(
	...
	// The role's resolved model and permissions, exactly as the terminal
	// relaunch applies them. effectiveAgentConfig alone is the project's view
	// and would drop the pin on every chat resume.
	agentConfig := mergeAgentConfig(
		effectiveAgentConfig(rec.Kind, project.Config),
		domain.AgentConfig{Model: strings.TrimSpace(rec.Metadata.Role.ResolvedModel)},
		rec.Metadata.Role.ResolvedPermissions,
		strings.TrimSpace(rec.Metadata.Role.RoleID) != "")
	// Rotate the spawn capability on every relaunch, as the terminal path does:
	// a killed session's old token must not survive the restart.
