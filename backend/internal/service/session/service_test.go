...
// ProjectOrchestrator answers the manager-owned ownership question delegation
// now asks instead of re-deriving it in the service.
func (f *fakeCommander) ProjectOrchestrator(_ context.Context, projectID domain.ProjectID) (domain.SessionRecord, bool, error) {
	...
	rec, ok := f.projectOrchestrator[projectID]
	return rec, ok, nil
}
...
func TestToAPIErrorMapsWorkspaceBranchSentinels(t *testing.T) {
	cases := []struct {
		...
		{"transient switch operation", fmt.Errorf("switch agent mer-1: %w", sessionmanager.ErrSwitchOperationInProgress), apierr.KindConflict, "SWITCH_IN_PROGRESS"},
		{"agent switch recovery", &sessionmanager.AgentSwitchRecoveryError{SessionID: "mer-1", SwitchID: "switch-1", State: domain.AgentSwitchStartingTarget}, apierr.KindConflict, "AGENT_SWITCH_RECOVERY_REQUIRED"},
		{"durable agent switch", fmt.Errorf("switch agent mer-1: %w", domain.ErrAgentSwitchInProgress), apierr.KindConflict, "AGENT_SWITCH_RECOVERY_REQUIRED"},
		{"legacy switch recovery", &sessionmanager.LegacySwitchRecoveryError{SessionID: "mer-1", GenerationID: "gen-1"}, apierr.KindConflict, "SWITCH_RECOVERY_REQUIRED"},
