...
type SpawnSessionRequest struct {
	...
	//
	// Mode is orthogonal to RoleID and may be sent with it: a role binds
	// harness, model and policy, not the interface. The daemon resolves the
	// role first and then preflights this mode against what the role resolved.
...
type DelegateTaskRequest struct {
	...
	// and that fallback keeps RoleID, because dropping the role to change
	// interface would silently turn a role-pinned worker into a free-form one.
...
type SetActivityRequest struct {
	...
	RoleResult            *domain.RoleResultReport `json:"roleResult,omitempty" description:"Structured result parsed from a role-pinned worker's trailing final-response envelope."`
