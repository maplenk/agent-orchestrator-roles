...
// ephemeral port rather than failing. Mutation authority over AO_DATA_DIR is
// **not** conferred by port binding: daemon.Run acquires an exclusive data-dir
// lease before opening the store or reconciling, so a second AO process exits
// before it reaches this bind. A port conflict here therefore means a non-AO
// process owns the configured port (or a race that already lost the lease).
// The actual bound port is logged ("daemon listening") and written to
// running.json for the supervisor/renderer.
func NewWithDeps(cfg config.Config, log *slog.Logger, termMgr *terminal.Manager, operatorSpawnToken string, deps APIDeps) (*Server, error) {
	...
	if err != nil {
		...
		// Configured port is taken (typically non-AO): lease holder may use ephemeral.
		// A second AO daemon never reaches here while holding mutation rights.
