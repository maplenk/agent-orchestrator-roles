...
func (s *Store) ProjectProviderEvent(
	...
	// GetSession returns sqlc.embed(sessions) in this fork, so the row wraps the
	// full session rather than a flat column list — which is what lets upstream's
	// new columns arrive without a hand-written adapter dropping them.
	if owner.Session.ControllerGeneration != generation {
		return false, nil
	}
