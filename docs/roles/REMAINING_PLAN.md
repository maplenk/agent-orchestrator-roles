...
| Phase 3A-2a (pause/resume surface) | **Landed + accepted** — operator-owned endpoints (LAN or operator credential; agent principals refused), contract in `PHASE3A_PAUSE_CONTRACT.md`, read-model pause view, four sentinels mapped |
...
| Phase 3A-2b (structured limit detection) | **Deferred behind the sync** — adapter seam first, every harness unsupported, real captured fixtures before enabling any harness, promote one at a time |
