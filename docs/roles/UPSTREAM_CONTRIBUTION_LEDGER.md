...
**Current extraction base:** local `upstream/main` at `3e2dbf424`.
...
At the refs above, the fork is 240 commits ahead and 3 behind, with 420 changed
...
Every non-trivial row retains one upstream issue and one PR.
...
These are audit candidates, not eight promised PRs. The Sync 3 execution
commits prove downstream intent, but each row must still compare the named
invariant and tests against current `upstream/main`. Until that evidence is
entered, coverage remains `unclassified` and no issue opens.

| Invariant | Chain | Downstream commits | Post-Sync-3 files | Handwritten LOC | #3548 coverage | Issue / PR | Schema effects | Migration pairing | Fingerprint shared since: upstream file / merge SHA / downstream sync | Validation | Status |
|---|---|---|---|---:|---|---|---|---|---|---|---|
| The exact encoded target launch command is proven to fit before source interaction, with ConPTY parity. | `SW1 launch capacity` | `f55cd351` | 3 runtime/test files | seed `+293/-69` | `unclassified` | none | None | n/a | n/a | Compare #3548 tmux launch construction; capacity boundary and source-alive refusal tests | `audit-required` |
| Claude and Codex continuation probes preserve `available` / `unavailable` / `unknown`; probe failure never means conversation absence. | `SW2 provider probes` | `7a533f53` | 9 adapter/port test files | seed `+334/-91` | `unclassified` | none | None | n/a | n/a | Compare native-session probe paths; unknown/config-directory/activity regressions | `audit-required` |
| The handoff artifact is finalized and capacity-checked while the source remains alive; source stop is strictly later. | `SW3 handoff ordering` | `bb3ec3be` | 4 composer/switch-engine files | seed `+392/-90` | `unclassified` | none | None | n/a | n/a | Compare saga order; human-draft fail-closed, oversized handoff, crash-boundary tests | `audit-required` |
| Native identity and session execution state are promoted only after acknowledgement for the exact AO session and target generation. | `SW4 exact acknowledgement` | `89f95742` | 5 handwritten manager/query/store test files | seed `+120/-16` | `unclassified` | none | None beyond upstream `0085_agent_switching.sql` | n/a unless extraction changes schema | n/a | Compare acknowledgement and scalar/native registry writes; stale/wrong-generation tests | `audit-required` |
| SQLite enforces the switch initial tuple, immutable provenance, legal state transitions, and coherent recovery tuple. | `SW5 persistence guards` | `5778f47c` | 10 handwritten domain/migration/store test files | seed `+833/-204` | `unclassified` | none | Fork pairing-only `9009_agent_switch_contract_guards.sql`: four guard triggers | Required: add inert pairing-only `9009 ↔ upstream TBD`; no legacy-version fields | `TBD / TBD / TBD` — currently fork-exclusive | Compare #3548 schema constraints; fresh/upgrade/adversarial transition tests and full pairing matrix | `audit-required` |
| The durable saga records exact authorized target harness/model, role snapshot, source generation, and target generation; retries cannot substitute intent. | `SW6 authorized intent` | `190ea997`, `26584306`, `e2a8c597` | 20 handwritten domain/manager/migration/query/store/service files | seed floor `+581/-121`, excluding characterization follow-up | `unclassified` | none | Fork pairing-only `9010_agent_switch_authorized_intent.sql`: saga/attempt columns and immutability triggers | Required: add inert pairing-only `9010 ↔ upstream TBD`; no legacy-version fields | `TBD / TBD / TBD` — currently fork-exclusive | Compare fingerprint/model fields in #3548; authorization-before-mutation and retry identity tests | `audit-required`; role-snapshot extraction additionally `blocked:RL2` |
| Ambiguous post-stop delivery exposes an exact, generation-fenced safe-recovery operation instead of generic resend. | `SW7 safe recovery` | `b31f406f`, `10d5d25f` | 2 primary manager files plus initiation/recovery guard tests | seed floor `+155/-0` | `unclassified` | none | None | n/a | n/a | Compare #3548 recovery operations; ambiguous ownership, restart, no duplicate delivery, drain-mode boot | `audit-required` |
| One canonical service/HTTP/CLI/UI contract exposes create, options, history, and exact recovery while preserving only a thin compatibility wrapper. | `SW8 recovery surfaces` | `6457c621`, `b4038015`, `c68f36e1` | 23 handwritten Go/TS/TSX source/test files | seed `+1,298/-235` | `unclassified` | none | None | n/a | n/a | Route/spec parity, typed envelopes, idempotency/history, generated-client-only renderer, real Electron flow | `audit-required`; split backend/API and desktop if isolated diff exceeds target |
...
2. Advance #3801 by requesting workflow approval at the 22:00 IST Discord
   sync; then address checks and request review.
3. Extract and open the data-directory lease issue/PR from current upstream.
4. Extract and open the xterm disposal-race issue/PR from current upstream.
...
Before each issue opens, update its row with the current upstream SHA, exact
isolated handwritten diff, final `covered`/`partial`/`absent` evidence,
dependency state, and—when schema-bearing—the activated migration filename and
complete pairing test evidence.
