...
Before an implementation issue opens, record the current upstream SHA,
behavioral evidence, dependency state, and a downstream seed estimate. After
the issue is assigned/approved, isolate the upstream-shaped branch and replace
the seed with the exact `upstream/main...branch` handwritten file count and LOC
before opening its PR. This ordering preserves CONTRIBUTING.md's requirement to
wait for assignment before building. Generated artifacts ship in the same PR
but never count toward the ceiling.
...
|---|---|---|---|---:|---|---|---|---|---|---|---|
| Missing tmux server is authoritative absence, while genuine probe failures remain unknown. | `R1 tmux absence` | `be4321d1`, `66d4ceb3`, `6a07d5d6`, `24906d35`; upstream extraction `976dc41ca` | 5 files: tmux runtime, outbound port, manager, tests | extraction `+159/-35` | `absent` | [#3800](https://github.com/Untrivial-ai/agent-orchestrator/issues/3800) / [draft #3801](https://github.com/Untrivial-ai/agent-orchestrator/pull/3801) | None | n/a | n/a | Focused tmux and manager absence/error tests; upstream CI | `open` — converted to draft and [claimed with the ordering error disclosed](https://github.com/Untrivial-ai/agent-orchestrator/issues/3800#issuecomment-5237737900). No assignment, review, or review request. All three PR workflows completed `action_required`, awaiting maintainer approval. Wait for assignment or explicit approval on #3800; only then approve workflows, address checks, and request review. Blocks only overlapping tmux-classification work. |
| Exactly one daemon owns and reconciles an AO data directory. | `R2 data-dir lease` | `9480bdc7`; upstream extraction `3db79ad0` | 6 files: `backend/internal/datadirlock/`, daemon wiring, HTTP fallback comments | extraction `+391/-8` | `absent` | [#3805](https://github.com/Untrivial-ai/agent-orchestrator/issues/3805) / [draft #3806](https://github.com/Untrivial-ai/agent-orchestrator/pull/3806) | None | n/a | n/a | Repeated and race lock tests, daemon/HTTP suites, serial full backend suite, build/vet/lint, Linux/Windows cross-compilation | `open` — draft preceded assignment; [claim/disclosure posted](https://github.com/Untrivial-ai/agent-orchestrator/issues/3805#issuecomment-5237740787). No assignment, review, or review request. All three PR workflows completed `action_required`, awaiting maintainer approval. Wait for an assignment or explicit retain/approve comment on #3805. If retained, address approved checks and only then request review; otherwise close #3806. |
| Replacing a live xterm handle cannot race queued viewport work against disposed terminal state. | `R3 xterm disposal` | `f8d31046`; upstream extraction `1d13dd72` | 2 files: `XtermTerminal.tsx`, real-xterm E2E | extraction `+33/-6`; component change `+14/-6` | `absent` | [#3803](https://github.com/Untrivial-ai/agent-orchestrator/issues/3803) / [draft #3804](https://github.com/Untrivial-ai/agent-orchestrator/pull/3804) | None | n/a | n/a | Frontend/E2E typecheck, 49 focused Vitest tests, exact real-xterm Playwright replacement regression | `open` — draft preceded assignment; [claim/disclosure posted](https://github.com/Untrivial-ai/agent-orchestrator/issues/3803#issuecomment-5237739727). No assignment, review, or review request. Both PR workflows completed `action_required`, awaiting maintainer approval. Wait for an assignment or explicit retain/approve comment on #3803. If retained, address approved checks and only then request review; otherwise close #3804. |
| Tmux servers are namespaced by resolved AO data directory so isolated installations cannot share processes. | `R4 tmux namespace` | `9f687d79` | 9 files across runtime selection, tmux socket/config, daemon, ports, service, tests | seed `+247/-8` | `absent` — at `c17b6c3f`, `runtimeselect.New` still constructs `tmux.Options{}` and the runtime has no socket selector; all data directories address the default server | pending issue / PR | None | n/a | n/a | `SocketForDataDir` stability/default-continuity tests; every-operation/attach selector tests; daemon wiring | `blocked:R1` — audit complete. Wait for #3801 because both touch tmux error/runtime files; after authorization, isolate the server-namespace core and exclude the downstream `ErrRuntimeSessionConflict` addition unless a distinct post-#3801 gap remains. |
| Every new Claude native conversation receives a caller-reserved stable UUID and cannot replay another session's identity. | `R5 Claude identity` | `15a4e6f0` | 7 code/test files across Claude adapter, agent port, manager, switching | seed `+220/-20`; remeasure only the narrowed gap | `partial` — upstream already has `AgentFreshNativeSessionIDProvider`, Claude UUID allocation, `LaunchConfig.NativeSessionID`, and fresh switch-target coverage; normal spawn and generic fresh relaunch omit `NativeSessionID` and still derive it from the reusable AO session ID | pending issue / PR | None | n/a | n/a | Reused-AO-ID replay regression; reserve and persist before initial launch; generic fresh relaunch gets a new durable ID; keep upstream switch coverage | `ready-for-issue` — do not copy downstream's duplicate allocator interface or already-covered switch implementation. Open/claim only the narrowed initial-spawn/relaunch gap, then wait for assignment before building. |
...
upstream-shaped diff is measured. On 2026-08-10 every row below was compared
with `upstream/main` at `c17b6c3f`: RL1 and RL5 are partial because upstream
already supplies reviewer read-only primitives and generic delegation,
respectively; RL2, RL3, RL4, RL6, RL7, RL8, and RL9 have no equivalent role
contract. Similar-sounding worker/orchestrator defaults, LAN bearer auth, and
reviewer adapter profiles are recorded as reusable foundations, not as
equivalent implementations.

| Invariant | Chain | Downstream commits | Post-Sync-3 files | Handwritten LOC | #3548 coverage | Issue / PR | Schema effects | Migration pairing | Fingerprint shared since: upstream file / merge SHA / downstream sync | Validation | Status |
|---|---|---|---|---:|---|---|---|---|---|---|---|
| Agent capabilities are daemon-authoritative, and a resolved read-only role is enforced at every AO-managed write boundary. | `RL1 capability contract` | `9fcd095c`, `4ab636fe` | 20 code/test files; core packages `roles/capabilities` and `roles/readonly` plus adapter/manager integration | seed `+750/-56` | `partial` — upstream has reviewer-specific read-only launch/restore enforcement and adapter capability surfaces, but no worker-role capability registry, no role policy, and no host-wide role read-only gate | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Registry matrix; extend rather than duplicate reviewer read-only primitives; negative worker write/delegation paths; reject unsupported harnesses | `design-approval` — after approval, the focused issue must describe the narrowed worker-role contract and wait for assignment before implementation. |
| A strict, versioned role map decodes and validates role, harness, model, permissions, and failover references without permissive fallback. | `RL2 role-map schema` | `31ca4e27`, `3c3aef51`, `663f9339`, `8bb1e3af` | 5 core schema/resolve files plus containment tests | seed floor `+739/-0` from `31ca4e27`; remeasure fixes | `absent` — `RoleMap`, `RoleBinding`, and `RoleExecutionPolicy` have no current-upstream equivalents at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | No database migration; project config remains durable JSON | n/a | n/a | Strict decode, referential validation, unreadable-config containment, legacy compatibility | `blocked:RL1` |
| Session role identity and immutable template content are pinned durably so later project edits cannot rewrite a running session's authority. | `RL3 template CAS` | `31ca4e27`, `1f80bdb5` | 18 source/test/migration files after separating RL2 | seed floor `+1,521/-97` from `31ca4e27`; remeasure host-ownership fix | `absent` — upstream has no `RoleID`, `RoleMap`, `TemplateArtifact`, or template-artifact store at `c17b6c3f`; generic system prompts and reviewer profiles do not pin semantic role identity or immutable template bytes to a session | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork `9000_session_role_fields.sql`: session role/template fields and `template_artifacts` | Required: historical repair row remains separate; add inert `9000 ↔ upstream TBD` pairing before PR opens | `TBD / TBD / TBD` — currently fork-exclusive | Fresh and upgrade migration matrix, template hash/CAS, role pin immutability, host-owned execution fields | `blocked:RL2` — audit complete. After RL2, isolate the migration/CAS core from later host-ownership corrections if the combined upstream-shaped diff exceeds the ceiling; pairing must be green before the schema PR opens. |
| Only a session holding a scoped, hashed spawn capability and valid operator/session principal can invoke `ao spawn --role`. | `RL4 authenticated role spawn` | `3269f389`, `132f94d6`, `75f5132c`, `81dfdb4d`, `93fcf1f8` | 32 code/test files across CLI, auth, daemon, controller, credential authority, runfile, desktop client | seed `+1,262/-347` | `absent` — upstream has no spawn capability, capability hash, session/operator principal, or spawn-specific credential authority at `c17b6c3f`; its LAN bearer and connection-token middleware are not session-scoped spawn authorization | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork `9001_session_spawn_capability_hash.sql`: `sessions.spawn_capability_hash` | Required: historical repair row remains separate; add inert `9001 ↔ upstream TBD` pairing before PR opens | `TBD / TBD / TBD` — currently fork-exclusive | Principal scoping, token secrecy, default-port and external CLI transport, replay and misuse errors | `blocked:RL3` — audit complete. Keep general LAN auth unchanged; extract only the spawn principal/capability boundary and land its inert pairing before opening the schema-bearing PR. |
| Strict delegation carries a role through service and HTTP/CLI boundaries and never accepts a free-form execution target in its place. | `RL5 delegation surface` | `5de2df0c`, `2198a521` | 5 primary code/test files plus read-only spawn regression | seed floor `+144/-5` | `partial` — upstream already ships `DelegateTask` through service, HTTP, CLI, and desktop and accepts free-form `RequestedAgent`/`Model`; the gap is the `RoleID` pass-through plus strict manager enforcement that rejects those execution overrides | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Happy path, missing/unknown role, read-only gate, typed daemon envelope, DTO drift | `blocked:RL4` — audit complete. Do not resubmit generic delegation; the child issue and diff must contain only role propagation, strict override rejection, and their contract tests. |
| The desktop composer selects an authorized role for strict projects and omits free-form agent/model fields. | `RL6 desktop role selection` | `2a46fde8`, `1f80bdb5`, `4703071f` | 12 TS/TSX and supporting backend test files before Sync 3 reconciliation | seed `+742/-67` | `absent` — upstream `TaskComposer` exposes agent/model controls and its delegate request has no `roleId`; the generated client contains no `roleMap` or `strictDelegation` contract at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Composer payload, defaults, unavailable role, project reload, real Electron flow | `blocked:RL5` — audit complete. Reuse the existing composer and project query; add only the strict-role branch and prove the forbidden agent/model fields are absent from its request. |
| Role-map updates use one daemon-authoritative compare-and-swap revision and return a conflict instead of losing concurrent edits. | `RL7 atomic role-map API` | `e65a1100` | 15 handwritten code/test files across domain, service, controller/spec source, SQL query/store | seed `+617/-12` | `absent` — upstream has no role-map route, `SetRoleMap`, `expectedRoleMapSha256`, or role-map conflict type at `c17b6c3f`; whole-project configuration updates do not provide subdocument CAS semantics | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | No new table/column; transactionally updates the existing project configuration after hashing its role-map subdocument | n/a | n/a | Expected-revision success/conflict, invalid map, store atomicity, generated-contract zero diff | `blocked:RL2` — audit complete. Keep it independent of RL3 storage and require one transaction to compare the role-map hash and persist the replacement without losing unrelated project config. |
| Desktop role-map editing preserves strict validation, degraded unreadable state, and CAS conflict recovery. | `RL8 role-map editor` | `a9fa4e0c`, `4703071f` | 7 TS/TSX source/test files plus eight locale catalogs | seed `+1,463/-31` excluding locale JSON | `absent` — upstream has no `ProjectRoleMapEditor`, `setProjectRoleMap` client, expected role-map digest, or role-map settings section at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Field validation, reorder/edit/delete, degraded state, stale revision, Settings integration, Electron flow | `blocked:RL7` — audit complete. Build on the accepted CAS API and keep the generic project settings/editor behavior outside this slice. |
| Shipped starter roles resolve to installed immutable profile artifacts and expose switchable, authorized defaults without granting capabilities implicitly. | `RL9 starter catalogue` | `13fc6acd`, `8071d4b1`, `e38ae32d`, `762ae160` | 19 Go/TS/JS source/test files plus five profile Markdown assets | seed `+842/-74` excluding profile Markdown | `absent` — upstream tracks no root `profiles/` assets and has no `StarterRoleMap`, role-profile staging, or semantic role catalogue at `c17b6c3f`; existing worker/orchestrator defaults and reviewer adapter profiles are non-equivalent foundations to reuse | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Packaging/staging, profile hash, starter-map validation, switch target resolution, installed-app smoke | `blocked:RL3` — audit complete. Preserve the separation between authored profile assets, packaging, and starter-map policy if the upstream-shaped diff no longer fits one reviewable slice. |
...
| Invariant | Chain | Downstream commits | Post-Sync-3 files | Handwritten LOC | #3548 coverage | Issue / PR | Schema effects | Migration pairing | Fingerprint shared since: upstream file / merge SHA / downstream sync | Validation | Status |
|---|---|---|---|---:|---|---|---|---|---|---|---|
| A pause is a durable, incident- and runtime-generation-fenced fact that blocks AO writes until the matching resume/restart transition. | `P1 pause primitive` | `506467f5`, `d0d7360a`, `8621303f`, `f39ee59b`, `e90b543a` | 17 primary handwritten domain/manager/guard/storage files | seed `+2,540/-132` before later fences | `absent` — at `c17b6c3f` there is no `SessionPause`, `PauseGuard`, `PauseSession`, incident fence, or pause migration; provider thread-resume states are unrelated | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork `9007_session_pause.sql`: `sessions.pause_json` | Required: historical repair row remains separate; add inert `9007 ↔ upstream TBD` pairing before PR opens | `TBD / TBD / TBD` — currently fork-exclusive | Fresh/upgrade, incident and generation mismatch, boot, chat/terminal input fences, resume idempotency | `split-required` — after umbrella approval, propose separate domain/storage and manager-fence children because the seed already exceeds 2,500. Open/claim the first focused issue and wait for assignment before isolating either branch. |
| Pause/resume remains an operator-only thin service/HTTP/CLI operation with typed errors and dead-runtime visibility. | `P2 pause API/CLI` | `930fa561`, `eee31d7b`, `7e388bf2` | 18 handwritten controller/service/client/test files | seed floor `+861/-23` | `absent` — current upstream exposes only agent/process resume; there is no incident-scoped pause/resume service method, HTTP route, DTO, or CLI command | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None beyond P1 | n/a | n/a | Auth, DTO parity, missing/expired incident, runtime-dead reads, daemon envelopes | `blocked:P1` |
| The desktop clearly exposes paused state and safe Resume/Restart actions without locally reimplementing policy. | `P3 pause desktop` | `d6d96c89`, `a3f5ec1b` (shared surface only) | 5 primary TS/TSX source/test files plus locales | seed floor `+284/-0` before Continue integration | `absent` — the upstream session read model has no durable pause fact or incident actions for the renderer to consume | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Inspector visibility, action states, chat input fencing, locale coverage, real Electron flow | `blocked:P2` |
| Typed limit events enter through a detector registry and router, but every production detector remains unsupported until separately approved. | `P4 limit seam` | `2e14852c`, `1d2f34f4` | 15 handwritten domain/router/manager/store test files | seed `+1,042/-42` | `absent` — no `LimitEnvelope`, detector registry, limit source, or limit router exists at `c17b6c3f`; captured vendor fixtures remain downstream evidence only | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Empty registry, stable source key, ownership/generation mismatch, duplicate event, unsupported harness | `design-approval` — schema-free and independent of P1 pairing, but implementation still waits for umbrella approval, a focused issue, and assignment. |
...
an issue is opened. The 2026-08-10 audit against `upstream/main` at
`c17b6c3f` classifies O1 and O3 as partial: upstream already has service-level
orchestrator reuse/clean replacement and generic fresh-relaunch foundations.
It has no manager-owned ownership gate, durable replacement intent, duplicate
owner reaping, orchestrator switch engine, or switch surface; O2, O4, and O5
remain absent.

| Invariant | Chain | Downstream commits | Post-Sync-3 files | Handwritten LOC | #3548 coverage | Issue / PR | Schema effects | Migration pairing | Fingerprint shared since: upstream file / merge SHA / downstream sync | Validation | Status |
|---|---|---|---|---:|---|---|---|---|---|---|---|
| The session manager is the sole authority for one active orchestrator owner per project across every launch, adoption, and teardown path. | `O1 ownership gate` | `6e9d58d5`, `b83ca31f`, `bb43c267`, `4411c0f5` | 6 handwritten manager/service test files | seed `+1,552/-238` | `partial` — upstream service paths already return an active orchestrator and clean replacement retires known owners, while `activeOrchestratorSessionID` reads one matching row; there is no manager-owned per-project gate serializing every launch, adoption, restore, and teardown path | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Concurrent starts, canonical workspace aliases, bypass paths, shared-workspace teardown | `blocked:RL3` — audit complete. Extract the manager admission/gate and bypass-path tests only; do not resubmit existing service reuse or clean-retirement behavior. |
| Database uniqueness and a fail-closed boot reaper reconcile duplicate owners before any daemon surface or restore is served. | `O2 uniqueness/reaper family` | `6b482589` through `abab7b3e` as inventoried in Sync 3 history | 30 handwritten daemon/runtime/manager/storage/migration test files | seed `+4,088/-381` | `absent` — upstream has no orchestrator reap queue, duplicate-owner migration, one-active unique index, or pre-serve queue drain at `c17b6c3f`; its general runtime observer/reaper handles liveness, not duplicate ownership reconciliation | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork `9004_one_active_orchestrator.sql`: reap queue and unique partial index | Required: historical repair row remains separate; add inert `9004 ↔ upstream TBD` pairing for the first schema child | `TBD / TBD / TBD` — currently fork-exclusive | Duplicate reconciliation, loser identity, restricted cleanup, restore ordering, unreadable evidence, second boot | `blocked:O1`; `split-required` — audit complete. Create separate database-reconciliation and boot/restore children, each below the ceiling, and pair only the exact first schema child. |
| Orchestrator replacement and in-place fresh conversation survive restart without losing ownership or lifecycle evidence. | `O3 replacement/fresh` | `cc10f4f8`, `e1c2a7dc`, `b3ce92a6`, `2806b18c` | 25 handwritten domain/handoff/manager/store/migration test files | seed `+2,202/-128` | `partial` — upstream verifies replacement sessions, has `RetireForReplacement`, rebinds project conversations, and supports generic fresh relaunch; it has no durable `OrchestratorReplacementIntent`, boot recovery for a zero-owner crash window, explicit `FreshOrchestratorConversation`, or its lifecycle kind | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork `9005_orchestrator_replacement_intent.sql` and `9006_lifecycle_ledger_orchestrator_fresh.sql` | Required: separate inert `9005` and `9006 ↔ upstream TBD` pairings; do not combine non-equivalent effects | each `TBD / TBD / TBD` — currently fork-exclusive | Pre/post-stop crash recovery, roleless legacy sessions, lifecycle kind, restart/idempotency | `blocked:O2`; audit complete, then size-audit. Preserve upstream retirement/relaunch primitives and split durable replacement intent from fresh-conversation lifecycle if their migrations are not physically equivalent. |
| An orchestrator may switch across authorized harnesses through its ownership protocol, never through the worker saga. | `O4 cross-harness backend` | `e3170a04`, `051db38b`, `97c7913c` | 15 handwritten manager/service test files | seed floor `+648/-71` | `absent` — upstream's #3548 worker saga explicitly rejects orchestrators and there is no `SwitchOrchestrator`, orchestrator pending-state protocol, or ownership-gated target authorization at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Uses fork `9002_lifecycle_ledger.sql` and `9003_session_switch_pending.sql`; no new schema expected | Pair `9002`/`9003` only if their exact effects are upstreamed; historical repair remains separate | each `TBD / TBD / TBD` — currently fork-exclusive | Worker/orchestrator route separation, exact role authorization, rollback/recovery, one-owner invariant | `blocked:O3`, `blocked:RL2`, and dependent SW policy rows — audit complete. Do not extend the worker saga to accept orchestrators. |
| CLI and desktop expose orchestrator switching only in the correct TUI mode and render daemon-authoritative recovery state. | `O5 switch surface` | `fa4d4f90`, `051db38b`, `31b6d7ef`, `72f274a7` | 24 handwritten Go/TS/TSX source/test files | seed floor `+1,357/-92` | `absent` — upstream has orchestrator listing and generic fresh-spawn surfaces but no orchestrator switch/fresh-conversation route, generated contract, CLI operation, renderer hook, or recovery view at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Route validation, chat-mode refusal/hidden control, pending state, typed errors, Electron flow | `blocked:O4` — audit complete. Reuse existing orchestrator and session UI structure while keeping policy and recovery state daemon-authoritative. |
...
detector is separately approved, promoted, and positively live-tested. The
2026-08-10 audit against `c17b6c3f` found no failover-policy, attempt,
Continue, preview, or desktop failover symbols upstream. F2 is nevertheless
partial because #3548 supplies the durable worker saga it adopts; F1, F3, and
F4 are absent, and F5 remains downstream-only by policy.

| Invariant | Chain | Downstream commits | Post-Sync-3 files | Handwritten LOC | #3548 coverage | Issue / PR | Schema effects | Migration pairing | Fingerprint shared since: upstream file / merge SHA / downstream sync | Validation | Status |
|---|---|---|---|---:|---|---|---|---|---|---|---|
| Failover policy previews an authorized next rung and stores each attempt, generation, role snapshot, and lifecycle record transactionally. | `F1 policy/store` | `cb333f07`, `b952a751`, `5b73d3c3`, `5d5a7866` | 11 handwritten domain/query/store/migration test files | seed `+1,002/-15` | `absent` — upstream has no `NextFailoverRung`, failover-attempt domain/store, attempt migration, or role-snapshot transaction at `c17b6c3f`; the worker-switch saga is not an attempt ledger or ladder policy | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | Fork pairing-only `9008_session_failover_attempts.sql` and `9011_failover_authorized_role_snapshot.sql` | Required: separate inert pairing-only `9008` and `9011 ↔ upstream TBD` rows; neither has legacy identities | each `TBD / TBD / TBD` — currently fork-exclusive | Transaction rollback, one attempt/rung, immutable snapshot, fresh/upgrade/idempotency, pairing-only matrix | `blocked:RL2`, `blocked:P1`, and accepted SW policy rows — audit complete. Keep `9008` attempt storage and `9011` authorized snapshot as separately pairable effects. |
| Manual Continue adopts or creates exactly one attempt and converges on the durable worker saga without spending another rung on retry. | `F2 manual Continue` | `01cf6580`, `45631c0d`, `866d3f08`, `b23e4970`, `ff5c3b1c` | 4 primary manager files plus focused recovery tests | seed floor `+2,240/-45` | `partial` — #3548 supplies the durable worker-switch saga, generation fencing, and recovery states, but upstream has no manual `ContinueFailover`, canonical failover-attempt adoption, rung reservation, or pause-lifting transaction | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None beyond F1 and accepted switch schema | n/a | n/a | Duplicate/concurrent Continue, crash before saga, completed retry, exhausted ladder, generation equality | `blocked:F1`; audit complete, then size-audit. Extract only the failover-to-saga convergence and reuse the accepted worker-switch engine rather than forking it. |
| Continue is exposed through thin service, HTTP, and CLI boundaries with preview and typed conflict/recovery envelopes. | `F3 Continue API/CLI` | `b4c71d07`, `d8421a32`, `064eaeef`, `7c018683` | 8 primary handwritten CLI/controller/service test files | seed floor `+699/-9` | `absent` — upstream has no `ContinueFailover`, `FailoverPreview`, Continue route/DTO, or failover CLI operation at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Preview/execute, missing target, conflict, completed result, row-level degraded reads, DTO parity | `blocked:F2` — audit complete. Keep the CLI thin and regenerate the accepted HTTP contract with zero subsequent diff. |
| The desktop pause surface offers daemon-authorized Continue targets and reflects attempt/saga progress without local escalation logic. | `F4 Continue desktop` | `a3f5ec1b` | 8 primary TS/TSX source/test files plus locales | seed `+888/-79` excluding locale JSON | `absent` — upstream has neither the durable pause view nor generated failover preview/attempt fields, Continue mutation, or renderer state at `c17b6c3f` | [umbrella #3802](https://github.com/Untrivial-ai/agent-orchestrator/issues/3802) | None | n/a | n/a | Target list, disabled/in-flight states, duplicate conflict, completed state, Electron flow | `blocked:F3` — audit complete. Render only daemon-authoritative preview and saga state; do not move rung selection into the desktop. |
...
upstream SHA, downstream seed estimate, final
`covered`/`partial`/`absent` evidence, dependency state, and—when
schema-bearing—the proposed pairing shape. After the issue opens, claim it and
wait for assignment plus any required non-trivial-work thumbs-up. Only then
build the upstream-shaped branch, record its exact handwritten diff and active
migration filename/test evidence, and open its PR.
