...
**Status:** **2B-0a and 2B-0b complete** (ownership gate, migration 0057 uniqueness +
...
now crash-consistent. **2B-3 is implemented; live acceptance is pending for the final MVP:** strict
...
migration 0057. There is one rule, in one place, and the database enforces the
...
**Prefer a constraint over a table.** Migration **0057** (originally 0046) adds a
...
Existing databases can already violate the constraint, so `0057` cannot simply
...
| **2B-3** | **Implemented; live acceptance pending.** In-place Codex↔Claude orchestrator switch through the manager-owned project gate, exact role-map authorization, existing switch fence/ledger/handoff, and same-generation recovery. Session/worktree/branch/role/template/permissions stay stable; credential rotates | code done | 2B-1 + final MVP strict-policy amendment |
