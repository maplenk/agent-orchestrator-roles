...
| Ordering | The queue drain is the **first** thing boot does after the shell closer is wired, and it is fatal — so the required sequence is: build the shell service *without* sweeping → `SetShellTerminalCloser` → **drain** → ordinary shell sweep → `Reconcile`/`RestoreAll` → browser, mobile, and API listeners. Fusing construction with the previous-run sweep put a best-effort pass ahead of the fail-closed gate; the surrounding listeners must sit behind it too, since an unconfirmed obligation must stop AO acting at all, not just stop it reconciling sessions. Pinned by `boot_order_test.go` |
...
| Constraint loss at spawn | SQLite reports the index as a bare `UNIQUE constraint failed: sessions.project_id (2067)` — the **column**, never the index name — so `CreateSession`/`UpdateSession` map it to `domain.ErrActiveOrchestratorExists` (409 `ACTIVE_ORCHESTRATOR_EXISTS`) instead of an opaque 500. `idx_sessions_one_active_orchestrator` is the only unique index on `sessions`, which is what makes the column unambiguous; adding another would require narrowing the predicate |
| **`MarkSpawned` cleanup** | `MarkSpawned` is the single write that adopts a launch — it clears `is_terminated` **and** records `RuntimeHandleID`/`RuntimeLaunchID` together — so until it commits there is a runtime no row names. Cleanup of that window must be probe-authoritative (`destroyRuntimeProbed`, `switch.go:596-611`), never a bare `Destroy`, and must close two distinct hazards. An **untracked runtime**: reconcile, `Kill` and the boot reaper all work from the recorded handle, so a survivor whose identity was never written is unreachable, not merely leaked — its identity is therefore recorded anyway, by a write that leaves `is_terminated` alone so it still succeeds when the index was the thing that failed. And a **phantom-live row**: on the `ResumeAgentWithMode` path the row is already ACTIVE and keeps the previous handle, which the teardown just killed, so it reads as live with no process behind it — and for an orchestrator it holds the project's single active slot, wedging the project. The session is therefore terminated, recoverable via `Restore` (`Branch`/`WorkspacePath`/`AgentSessionID` all preserved). The workspace teardown is likewise gated on **confirmed** death, since an unconfirmed runtime may still be writing to it. The one exemption is the switch saga (`KeepSessionOnLaunchFailure`): `RecoverSwitchFromPostStop` refuses a terminated session, so terminating there would swap a recoverable `ErrSwitchPostStop` for a dead end — the runtime is still reaped, only the row's terminal state is left to the saga |
| **Compensating writes are not best-effort** | The rollback's own writes can fail for the very store condition that rejected `MarkSpawned`, so logging and discarding them lets a caller report a broken invariant as handled. Both are returned as `ErrLaunchCleanupUnresolved`: a survivor that could not be **recorded** (nothing can ever find it), and a session that could not be **terminated** (it holds the orchestrator slot with nothing behind it). Ordering is part of the contract — the stale runtime identity is cleared only *after* termination has actually persisted, because clearing it on a still-active row leaves an orchestrator occupying the project's only slot with an empty handle, which runtime reconciliation skips entirely. Clearing the identity itself stays best-effort by contrast: a terminated row naming a dead handle costs one redundant probe |
| **Evidence preservation** | Reconciliation erases the losers' `runtime_handle_id`/`runtime_launch_id` and restore markers, so the reaper would have nothing authoritative to probe. Migration 0046 therefore captures every loser into `orchestrator_reap_queue` — session id, project id, **original** handle and launch id, workspace path — *before* clearing anything. Semantics are **delete-on-authoritative-reap**: a row's presence means "this execution is still owed a confirmed death", enforced with `ON DELETE RESTRICT` — deliberately *not* 0025's `CASCADE`, since an obligation to kill an OS process does not become obsolete when its row disappears. The reaper must confirm death and remove the entry itself before the session can be deleted. **The reaper must also fail boot loudly if the queue table is absent**, rather than treating "no table" as "nothing owed". An empty captured handle means the row genuinely had none recorded, and the reaper must fall back explicitly rather than reading it as "already dead" — but **not** to the raw session id: adapters derive their handle (tmux sanitizes ids that are too long or hold characters it rejects, and its `IsAlive` validates whatever handle it is given), so the raw id either probes a runtime that never existed or is rejected outright. The fallback resolves through `ports.RuntimeSessionHandleResolver`, and a runtime that cannot answer fails the obligation rather than guessing |
| **Drain failures fail closed** | Migration 0046 has *already* terminated the losers and released their claims by the time the reaper runs, so "refuse to retire" is not the available lever — the row state is settled and only the **processes** are outstanding. The lever is the queue and boot itself: an unconfirmed runtime or shell **retains the obligation and aborts startup**. `drainScopedShells` is deliberately best-effort for interactive cleanup — an unclosable shell is logged and skipped — but the reaper propagates that same condition as a fatal error. It is therefore wired as its own call **before** `Reconcile`, not inside it, because `Reconcile`'s failures are logged and boot continues (`daemon.go`). Two fail-open holes on that path had to be closed for the shell half to mean anything: `destroyConfirmed` probed `IsAlive` only after a `Destroy` **error**, so a `Destroy` that reported success while the pty survived deleted the row and let the obligation discharge — the probe is now unconditional and is the authority; and `beginShellTerminalTeardown` answers "no closer wired" with success, which is right for ordinary teardown but would let the reaper discharge an obligation having checked nothing, so it uses a required variant that treats an unwired closer as a failure |
...
| **2B-0b** | **Coordinator uniqueness**: migration 0046 partial unique index, plus the reconciliation spec in §3.2b (deterministic survivor, marker neutralization, probe-authoritative reap, restore preflight). Closes **D1**. **Remaining acceptance criterion — boot must not serve on an unresolved launch cleanup.** `Reconcile` runs the terminated-session reap pass *before* `RestoreAll`, so a relaunch whose runtime survived teardown is not swept until the next restart. Two of the three hops now carry the signal: the launch rollback returns `ErrLaunchCleanupUnresolved` for **every** unconfirmed death (recorded or not — recording makes a survivor reapable *eventually*, not gone), and `RestoreAll` collects and returns those instead of logging-and-skipping, while still restoring every other session. The remaining hop is `Reconcile` → daemon: today the daemon logs `Reconcile`'s error and serves, so the gated-`RestoreAll` slice must make this one **fatal**, alongside the reap-queue drain. Until that lands the slice is not independently production-safe | 1–2 d | 2B-0a |
